...
**設計フェーズ 3/N: Implementation Plan のみ。** 本書は DB migration・CLI・export schema・テストコードの変更を伴わない。実装は本書 merge 後の Phase 1 PR 以降。

| ドキュメント | 役割 |
|---|---|
| [estimate-model.md](estimate-model.md) | 責務・境界（Planned Money vs Actual Money） |
| [estimate-entity-design.md](estimate-entity-design.md) | DDL・CLI 案・validation・将来 export v6 |
| **本書** | 実装計画（If we build it, how） |
| `estimate-post-implementation-review.md`（将来） | 実装後レビュー（Phase 5） |
...
Phase 1 Implementation    → migration + estimate CRUD + CLI（未着手）
Phase 2–5                 → export v6 / stats / replicate / review（未着手）
```
...
影響範囲が広いため **1 PR に詰め込まない**。以下の 5 Phase で進める。

| Phase | 内容 | 想定 PR |
|---|---|---|
| **Phase 1** | DB migration + `src/money.rs` + estimate CRUD + CLI + cascade + tests + command-reference | **最初の実装 PR** |
| **Phase 2** | export / import **schema v6**、`validate-export`、`trip diff` | 2 本目 |
...
✗ release / tag
✗ estimate-post-implementation-review.md
```
...
| 現行 export | Phase 2 完了まで **schema v5 のまま** |
...
本 Implementation Plan **および Phase 1 以外** で扱う / 本書 PR では **行わない**:
