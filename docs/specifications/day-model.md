...
| `day swap <trip_id> <day_a> <day_b>` | 実装済 | 2 Day の plan payload を入れ替え（Itinerary / title / summary / Day-level notes） |
...
```text
Day shell（番号・日付・id）は固定
plan payload（Itinerary / title / summary / Day-level notes）だけ交換
```

- `day_number` / `trip.start_date` / `trip.end_date` / `days.id` / `created_at` は変更しない
- 交換対象: `itinerary_items`（`day_id` + export 互換用 `day` 列）、`days.title`、`days.summary`、Day-level `notes`（`owner_type = day`）
- Trip Note / Itinerary Note は個別に入れ替えない（Itinerary Note は Itinerary と一緒に移動）
- `UPDATE ... CASE` と Note owner の段階的 UPDATE を **トランザクション内** で実行。失敗時は rollback
...
- `day swap` は DB 内の plan payload を更新（Itinerary / Day title・summary / Day-level notes）。export JSON の top-level 構造は変わらない
