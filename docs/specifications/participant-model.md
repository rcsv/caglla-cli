...
```text
Participant (v2) is a Trip-scoped participation record —
a row that identifies and displays someone who takes part in that trip.
It is not the canonical master record for the person themselves.
```
...
#8（Entity Design）および Person / Trip 境界補正を前提に、#9 で確定する主な項目:

- `participants` を **Trip-scoped participation record** として実装する前提の明文化
- migration ファイル名・`db.rs` 組み込み（`person_id` / `persons` テーブルは **含めない**）
- `src/participant.rs` 関数一覧とテスト矩阵
- export v4 フィールド正式定義と `validate-export` ルール
- CLI オプション詳細・エラーメッセージ一覧
- `trip duplicate` / import 順序の手順書
- 将来 `person_id` 追加・`trip_participants` rename の余地を壊さない設計メモ
