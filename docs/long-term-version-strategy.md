...
## v3.7 — Receipt Assignment / Trash workflow（**v3.7.0 release preparation 中**）
...
```text
実装完了（776bab6）
release preparation 中（release notes / version bump）
```
...
設計系列: Workflow Design → Implementation Plan → 実装（776bab6）→ Release v3.7.0。
...
| **v3.7** | Receipt assign / trash | assign / trash / restore + pending sum + export v8 — **v3.7.0 release preparation 中** |
